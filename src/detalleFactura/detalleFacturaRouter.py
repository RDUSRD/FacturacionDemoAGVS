...
from src.detalleFactura.detalleFacturaService import (
...
@router.post("/create")
def create_or_get_detalle_factura_endpoint(detalle_factura_data: DetalleFacturaSchema, db: Session = Depends(get_db)):
    return get_or_create_detalle_factura(db, detalle_factura_data)

@router.put("/{detalle_factura_id}")
def update_detalle_factura_endpoint(detalle_factura_id: int, detalle_factura_data: DetalleFacturaUpdateSchema, db: Session = Depends(get_db)):
    detalle_factura = update_detalle_factura(db, detalle_factura_id, detalle_factura_data)
    if not detalle_factura:
        raise HTTPException(status_code=404, detail="Detalle de factura no encontrado")
    return detalle_factura