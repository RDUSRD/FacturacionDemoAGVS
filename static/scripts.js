...
async function submitForm(event, endpoint) {
    ...
    if (endpoint.includes("facturas") || endpoint.includes("debit_notes") || endpoint.includes("credit_notes")) {
        jsonData.operations = [];
        ...
        operations.forEach(op => {
            jsonData.operations.push({
                ...
                quantity: op.querySelector("input:nth-child(4)").value ? parseInt(op.querySelector("input:nth-child(4)").value) : null,
    ...
    if (endpoint.includes("debit_notes") || endpoint.includes("credit_notes") || endpoint.includes("retention_receipts")) {
        jsonData.related_document_id = parseInt(formData.get("related_document_id"));
    }
